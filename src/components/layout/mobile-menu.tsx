...
export function MobileMenu({ categories }: { categories: Category[] }) {
  return (
    <Dialog.Root>
      <Dialog.Trigger asChild><button type="button" className="inline-flex size-10 items-center justify-center rounded-full border border-border bg-card xl:hidden" aria-label="فتح القائمة"><Menu className="size-5" aria-hidden="true" /></button></Dialog.Trigger>
      <Dialog.Portal>
        ...
        <Dialog.Content dir="rtl" className="fixed inset-y-0 right-0 z-50 w-[min(88vw,390px)] overflow-y-auto border-l border-border bg-background p-5 shadow-[var(--shadow-float)]">
          ...
          <nav className="mt-6 grid gap-1" aria-label="التنقل على الجوال والأجهزة اللوحية">{links.map(([label, href]) => <Dialog.Close asChild key={href}><Link href={href} className="rounded-xl px-4 py-3 text-sm font-semibold hover:bg-accent">{label}</Link></Dialog.Close>)}</nav>
