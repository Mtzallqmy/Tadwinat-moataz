...
export async function submitContactAction(_previous: ContactActionState, formData: FormData): Promise<ContactActionState> {
  ...
  if (name.length < 1 || name.length > 120 || subject.length < 1 || subject.length > 240 || message.length < 1 || message.length > 10_000 || !/^[^\s@]+@[^\s@]+\.[^\s@]+$/u.test(email)) {
    return { ok: false, message: "تحقق من الاسم والبريد والموضوع والرسالة." };
  }
  const supabase = createPublicClient();
  const { error } = await supabase.rpc("contact_submit", { p_name: name, p_email: email, p_subject: subject, p_message: message, p_honeypot: honeypot });
  if (error) {
